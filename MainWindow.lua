...
function MainWindow:RenderPlayerRow(row, player, rowIndex, cardWidth)
    ...
    local rollWidth = 24
    local nameWidth = math.max(34, rowWidth - choiceWidth - rollWidth - 2)
...
function MainWindow:ShouldHideItemName(frameWidth)
    return frameWidth < ns.Constants.HIDE_ITEM_NAME_WIDTH
end

function MainWindow:RenderCard(card, record, topOffset, cardWidth, hideItemName)
    ...
    if hideItemName then
        card.status:SetPoint("LEFT", card.icon, "RIGHT", 4, 0)
        if ownChoiceTexture then
            card.status:SetPoint("RIGHT", card.ownChoiceIcon, "LEFT", -3, 0)
        else
            card.status:SetPoint("RIGHT", card, "RIGHT", -4, 0)
        end
    elseif ownChoiceTexture then
        card.status:SetPoint("RIGHT", card.ownChoiceIcon, "LEFT", -3, 0)
    else
        card.status:SetPoint("TOPRIGHT", -4, -7)
    end
    ...
    card.itemName:SetShown(not hideItemName)
...
function MainWindow:Refresh()
    ...
    local hideItemName = self:ShouldHideItemName(self.frame:GetWidth())
    local offset = 0
    ...
    for index, record in ipairs(records) do
        ...
        offset = offset + self:RenderCard(card, record, offset, cardWidth, hideItemName) + 1
    end
...
function MainWindow:Initialize()
    ...
    local clearHistoryButton = CreateFrame("Button", nil, frame)
    clearHistoryButton:SetSize(16, 16)
    clearHistoryButton:SetPoint("RIGHT", optionsButton, "LEFT", -2, 0)
    clearHistoryButton:SetNormalTexture("Interface\\Buttons\\UI-StopButton")
    clearHistoryButton:SetHighlightTexture("Interface\\Buttons\\UI-StopButton")
    clearHistoryButton:GetHighlightTexture():SetAlpha(0.35)
    clearHistoryButton:SetScript("OnClick", function()
        ns.Database:ClearHistory()
        self:Refresh()
    end)
    clearHistoryButton:SetScript("OnEnter", function(button)
        GameTooltip:SetOwner(button, "ANCHOR_TOP")
        GameTooltip:SetText(ns.L.CLEAR_HISTORY)
        GameTooltip:Show()
    end)
    clearHistoryButton:SetScript("OnLeave", function()
        GameTooltip:Hide()
    end)
    ...
    local title = frame:CreateFontString(nil, "ARTWORK", "GameFontHighlightSmall")
    title:SetPoint("LEFT", topBar, "LEFT", 7, 0)
    title:SetPoint("RIGHT", clearHistoryButton, "LEFT", -5, 0)
    title:SetJustifyH("LEFT")
    title:SetWordWrap(false)
    title:SetText(ns.L.TITLE)
    title:SetTextColor(0.55, 0.62, 0.72, 0.58)

    local dragArea = CreateFrame("Frame", nil, frame)
    dragArea:SetPoint("TOPLEFT", 2, -2)
    dragArea:SetPoint("TOPRIGHT", clearHistoryButton, "TOPLEFT", -2, 0)
